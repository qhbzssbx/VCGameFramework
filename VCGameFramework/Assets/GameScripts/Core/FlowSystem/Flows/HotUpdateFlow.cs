...
namespace Game.Core.FlowSystem.Flows
{
    /// <summary>
    /// 热更新检查状态
    /// </summary>
    ...
    public class HotUpdateFlow : BaseMainFlow
    {
        ...
        protected override async UniTask OnEnterInternal(FlowContext context)
        {
            ...
            {
                _logService.Error($"热更新流程发生错误: {ex.Message}");
            ...
            try
            {
                ...
                if (latestVersionInfo != null)
                {
                    ...
                    if (IsVersionNewer(latestVersionInfo.Version, currentVersion))
                    {
                        ...
                        _updateMessage = $"发现新版本 {latestVersionInfo.Version}\\n{latestVersionInfo.Description}";
        ...
        private async UniTask WaitForUserConfirmation()
        {
            ...
            // 监听用户输入
            _inputManager.OnConfirmPressed += OnUpdateConfirmed;
            
            // 等待用户做出选择
            while (!_userConfirmed && _currentState == HotUpdateState.CheckingVersion)
            {
