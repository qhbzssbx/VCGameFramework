 ...
 * ```csharp
 * // 显示UI
 * await UIManagerService.Instance.ShowAsync<MyUI>("MyUIPrefab", "参数1", "参数2");
 * 
 * // 隐藏UI
 * await UIManagerService.Instance.HideAsync<MyUI>();
 * 
 * // 切换显示状态
 * await UIManagerService.Instance.ToggleAsync<MyUI>("MyUIPrefab");
 * 
 * // 检查UI状态
 * bool isShowing = UIManagerService.Instance.IsShowing<MyUI>();
 * 
 * // 获取UI实例
 * var myUI = UIManagerService.Instance.GetPanel<MyUI>();
 * ```
 * 
 * 🎨 动画配置：
 * 在Inspector中可以配置：
 ...
 *             UIManagerService.Instance.ShowAsync<GameUI>("GameUI").Forget();
 *         });
 *     }
 * }
 * 
